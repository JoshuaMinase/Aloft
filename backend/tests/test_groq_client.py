...
from app.clients.groq import GROQ_API_URL, GroqClientError, chat_completion
...
def _fake_settings(**overrides) -> Settings:
    defaults = {"groq_api_key": "test-key", "groq_model": "test-model"}
...
async def test_chat_completion_raises_when_api_key_missing(monkeypatch):
    _mock_settings(monkeypatch, groq_api_key=None)

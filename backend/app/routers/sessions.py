...
async def start_session(
    ...
    session = await create_session(db, body.route_key)
...
async def update_position(
    ...
    if next_poi is None:
        await record_position_and_narration(db, session_id, body.lat, body.lng, None)
        return PositionUpdateResponse(triggered=False)

    story = await get_story(db, next_poi.source_id, body.language)
    await record_position_and_narration(db, session_id, body.lat, body.lng, next_poi.source_id)
